...
export default function MetricSlider({
  ...
  function handleKeyDown(event: React.KeyboardEvent<HTMLInputElement>): void {
    switch (event.key) {
      ...
      case 'ArrowRight':
        ...
        onChange(Math.min(MAX_METRIC_SCORE, value + 1) as MetricScore);
        break;
      case 'ArrowDown':
      case 'ArrowLeft':
        event.preventDefault();
        onChange(Math.max(0, value - 1) as MetricScore);
        break;
      case 'Home':
        event.preventDefault();
        onChange(0 as MetricScore);
        break;
      case 'End':
        event.preventDefault();
        onChange(MAX_METRIC_SCORE as MetricScore);
        break;
      case 'PageUp':
        event.preventDefault();
        onChange(Math.min(MAX_METRIC_SCORE, value + 1) as MetricScore);
  ...
  const description = values?.[value]?.description ?? 'No description available';
